...
!  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  do i=1,nAgWells
    ...
    write(536,'(3I10,ES16.5)')ag_wells(i)%layer, ag_wells(i)%well_row, ag_wells(i)%well_col, -1*ag_wells(i)%monthly_rate
  enddo

  if (nSpecWells>0) then
    do i=1, nSpecWells
      if (abs(spec_wells(i)%specified_rate) > 0.0) then
        write(536,'(3I10,ES16.6)')spec_wells(i)%layer, spec_wells(i)%well_row, spec_wells(i)%well_col, spec_wells(i)%specified_rate
      endif
    enddo
  end if
  ...
  if (nMFRWells>0) then
    do i=1, nMFRWells
      if (mfr_wells(i)%specified_rate > 0.0) then
        write(536,'(3I10,ES16.6,10x)') 1, mfr_wells(i)%well_row, mfr_wells(i)%well_col, mfr_wells(i)%specified_rate
      endif
    enddo
  end if
